...
export default class Document extends React.Component {
    ...
    loginnn = (event) => {
        ...
        fetch(`http://localhost:3001/signup`, {
            ...
            .then(res => res.json())
            .then(data => this.setState({logged_in: data, favorites: data.user.user.user_teams}, () => console.log(this.state)))
            
    }
    addFav = (team) => {
      if (this.state.logged_in !== '') {
      if (this.state.favorites.find(t => t === team)) {
        alert("Already one of your favorites")
      }  else {
      this.setState({favorites: [...this.state.favorites, team]})
      let user_team = {user_id: this.state.logged_in.user.user.id, team: team}
      fetch(`http://localhost:3001/user_teams`, {
            ...
            .then(res => res.json())
      }} else{ alert("Must be logged in to add to favorites.")}
    }
    ...
    render() { 
        return(  <Router>
            <div>
            <Navbar />
            <Route exact path="/" ><App logged={this.state.logged_in} favorites={this.state.favorites} addFav={this.addFav}/></Route>
